...
def retrieve_exam_years(course_code: str) -> List[str]:
    ...
    if response.status_code != 200:
        warn(
            f"WARN: Response was not 200! For course {course_code}, was {response.status_code}"
        )
    soup = bs4.BeautifulSoup(response.text, HTML_PARSER)
    select_tag_exam_year: Optional[bs4.element.Tag] = soup.find(
        attrs={"id": "selectedYear"}
    )
    if select_tag_exam_year is None:
        warn(
            f"Couldn't find the exam year select element, fagkode={course_code}, returning empty list"
        )
        return []

    exam_years: List[str] = [
        option["value"] for option in select_tag_exam_year.find_all("option")
    ]
...
def retrieve_exam_type_of_years(
    course_code: str, years: List[str], sleep_time_mean_ms=0.05
...
def course_has_digital_exam_semester(
    course_code: str, year: str, semester_code: str
) -> bool:
    semester_code = (
        "Spring"
        if semester_code == "V"
